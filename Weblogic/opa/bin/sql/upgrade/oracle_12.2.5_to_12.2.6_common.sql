...
ALTER TABLE AUTHENTICATION MODIFY user_name VARCHAR2(255 CHAR);
ALTER TABLE DEPLOYMENT_ACTION_LOG MODIFY user_id VARCHAR2(255 CHAR);

-- Add new columns
CALL opa_add_column_if_not_exists('AUTHENTICATION', 'user_type', 'NUMBER(5) DEFAULT 0 NOT NULL');

-- just in case this script has already been run, temporarily create the onld rows
CALL opa_add_column_if_not_exists('DEPLOYMENT', 'activator_id', 'INT');
CALL opa_add_column_if_not_exists('DEPLOYMENT_ACTIVATION_HISTORY', 'authentication_id', 'INT');
CALL opa_add_column_if_not_exists('DEPLOYMENT_VERSION', 'creator_authentication_id', 'INT');
CALL opa_add_column_if_not_exists('PROJECT_VERSION', 'creator_authentication_id', 'INT');
CALL opa_add_column_if_not_exists('DEPLOYMENT', 'user_name', 'VARCHAR2(255 CHAR)');
CALL opa_add_column_if_not_exists('DEPLOYMENT_VERSION', 'user_name', 'VARCHAR2(255 CHAR)');
CALL opa_add_column_if_not_exists('DEPLOYMENT_ACTIVATION_HISTORY', 'user_name', 'VARCHAR2(255 CHAR)');
CALL opa_add_column_if_not_exists('PROJECT_VERSION', 'user_name', 'VARCHAR2(255 CHAR)');
CALL opa_add_column_if_not_exists('SECURITY_TOKEN', 'is_long_term', 'NUMBER(5) DEFAULT 0 NOT NULL');
...
INSERT INTO CONFIG_PROPERTY (config_property_name, config_property_int_value, config_property_type, config_property_public, config_property_description)
    (SELECT 'externally_managed_identity', 0, 1, 0, '0 = disable, 1 = enabled. Enable for users which are managed and authenticated externally' FROM DUAL 
        WHERE NOT EXISTS (SELECT * FROM CONFIG_PROPERTY WHERE config_property_name='externally_managed_identity'));
...
INSERT INTO CONFIG_PROPERTY (config_property_name, config_property_str_value, config_property_type, config_property_public, config_property_description)
    (SELECT 'external_logout_url', null, 0, 1, 'Used for logging out of externally managed opa site' FROM DUAL 
        WHERE NOT EXISTS (SELECT * FROM CONFIG_PROPERTY WHERE config_property_name='external_logout_url'));

INSERT INTO CONFIG_PROPERTY (config_property_name, config_property_int_value, config_property_type, config_property_public, config_property_description)
    (SELECT 'file_attach_name_max_chars', 100, 1, 1, 'Maximum number of characters allowed in a file attachment name' FROM DUAL 
        WHERE NOT EXISTS (SELECT * FROM CONFIG_PROPERTY WHERE config_property_name='file_attach_name_max_chars'));

-- remove user id columns
CALL opa_drop_column_if_exists('DEPLOYMENT', 'activator_id');
CALL opa_drop_column_if_exists('DEPLOYMENT_ACTIVATION_HISTORY', 'authentication_id');
CALL opa_drop_column_if_exists('DEPLOYMENT_VERSION', 'creator_authentication_id');
CALL opa_drop_column_if_exists('PROJECT_VERSION', 'creator_authentication_id');
