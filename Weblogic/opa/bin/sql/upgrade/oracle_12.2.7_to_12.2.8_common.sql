...
CALL opa_create_obj_if_not_exists('CREATE SEQUENCE ssl_certificate_seq START WITH 1 INCREMENT BY 1');

-- Drop the SSL_PUBLIC_CERTIFICATE table in case it has been created incorrectly
-- in this upgrade at step 12.2.5 -> 12.2.6
CALL opa_drop_table_if_exists('SSL_PUBLIC_CERTIFICATE');
...
-- Add new column
CALL opa_add_column_if_not_exists('DATA_SERVICE', 'use_trust_store', 'NUMBER(5) DEFAULT 0 NOT NULL');
CALL opa_add_column_if_not_exists('DEPLOYMENT', 'activated_javascript_sessions', 'NUMBER(5) DEFAULT 0 NOT NULL');
CALL opa_add_column_if_not_exists('DEPLOYMENT_ACTIVATION_HISTORY', 'status_javascript_sessions', 'NUMBER(5) DEFAULT 0 NOT NULL');
